...
def get_overall_accuracy_dict_score():
    dict_accuracy_overall = {}
    base_path = "../output/inference_sirion_v1/accumulated_accuracy/"
...
def get_overall_latency_dict_score():
    dict_latency_overall = {}
    base_path = "../output/inference_sirion_v1/"
...
def show_dashboard(folder_name='data/'):
        #######################################################
        ...
        tab1, tab2, = st.tabs([ "Overall model Comparative Analysis", "Sirion Query to Sql Models Evaluation",])
        ...
        with tab2:
            ...
            input_dataset_file =  base_path+ context_data_option + ".csv"       
            
            df_selected = pd.read_csv(input_dataset_file)

            # filtered_df = df_selected[df_selected['expected_sql_valid']==True]
            grouped = df_selected.groupby('Expected classification')
